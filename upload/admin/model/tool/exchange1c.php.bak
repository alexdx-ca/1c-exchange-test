...
class ModelToolExchange1c extends Model {
	...
	public function queryOrders($params) {
		...
		if ($query->num_rows) {
			...
			foreach ($query->rows as $orders_data) {
                ...
                $shipping_postcode = empty($order['shipping_postcode']) ? '' : ', '.$order['shipping_postcode'];
                $shipping_zone = empty($order['shipping_zone']) ? '' : ', '.$order['shipping_zone'];
				...
				$document['Документ' . $document_counter]['Контрагенты']['Контрагент'] = array(
					...
					,'Адрес' => array(
						'Представление'	=> $order['shipping_address_1'].$shipping_city.$shipping_postcode.$shipping_zone.$shipping_country
					)
					,'Контакты' => array(
						'Контакт1' => array(
							'Тип' => 'ТелефонРабочий'
				...
				foreach ($products as $product) {
                    ...
                    $productTotalSumm=$product['total']+$productTotalSumm;
                ...
                $priceDelivery = $order['total']-$productTotalSumm;
                if ($priceDelivery>0) {
                    $document['Документ' . $document_counter]['Товары']['Товар' . $product_counter] = array(
                        'Ид'             => 'ORDER_DELIVERY'
                    ,'Наименование'   => 'Доставка'
                    ,'ЦенаЗаЕдиницу'  => $priceDelivery
                    ,'Количество'     => 1
                    ,'Сумма'          => $priceDelivery
                    ,'Артикул'	  => ''
                    );
                ...
                $order_status_id = $this->db->query("SELECT order_status_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$orders_data['order_id'] . "'");
                ...
                $document['Документ' . $document_counter]['ЗначенияРеквизитов']['ЗначениеРеквизита' . $details_counter] = array(
                    'Наименование'   => 'Статус заказа'
                ,'Значение'  => $order_status->row['name']// $this->model_sale_order->getOrderStatus($orders_data['order_id'])
                );
	...
	public function queryOrdersStatus($params){
		...
		if ($query->num_rows) {
			foreach ($query->rows as $orders_data) {
				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$params['new_status'] . "', date_modified = NOW() WHERE order_id = '" . (int)$orders_data['order_id'] . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$orders_data['order_id'] . "', order_status_id = '" . (int)$params['new_status'] . "', notify = '" . (isset($params['notify']) ? (int)$data['notify'] : 0) . "', comment = '', date_added = NOW()");
			}
		}
	...
	*/
    ...
    /**
     *Обновляем продукт и если его уже удалили перезаписываем
     */
    private function updateAndRebild($data, $product, $product_id = false, $language_id){
        $this->updateProduct($product, $product_id, $language_id);
        $productRecord = $this->model_catalog_product->getProduct($product_id);
        if ($productRecord){
            $this->updateProduct($product, $product_id, $language_id);
        }else{
    ...
    /**
     * Получает product_id по наименованию
     *
     * @param 	string
     * @return 	int|bool
     */
    private function getProductByName($name, $language) {
        if ($name==''){	return false;}

        $query = $this->db->query("SELECT product_id FROM `" . DB_PREFIX . "product_description` WHERE `name` = '" . $this->db->escape($name) . "'"." and 'language_id'= ".$language);
    ...
    private function get1CProductIdByProductId($product_id) {
			...
			if ($query->num_rows) {
				...
				$idIn1C = 'opencart'.md5($product_id['product_id']+$product_id['model']+$product_id['name']).$guid;
				$query = $this->db->query('INSERT INTO ' . DB_PREFIX . 'product_to_1c set `product_id` = "' . $product_id['product_id'] .'" ,`1c_id` = "' . $idIn1C . '"' );
				return $idIn1C;
			}
    }
